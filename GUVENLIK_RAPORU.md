...
### 2) Onceki oy kaydinin degistirilmesi engellendi

`saveVoteAtomically` icinde `voteDoc.exists` durumunda mevcut kaydi `update` etmek yerine sadece mevcut `cardNumber` geri donduruluyor.

Bu degisiklik, ayni fingerprint ile mevcut oy dokumaninin ustune yazilmasini engeller.

> Dosya: `js/antifraud.js` (`saveVoteAtomically`)
...
1. **Firestore security rules denetimi**
   - `list: false`
   - `update/delete: false` (oy belgeleri icin)
...
Mevcut mimari hobi/etkinlik olceginde guclu bir anti-fraud tabani sunuyor. Bu PR ile:

- istemci tarafi manipule edilmis oy verileri filtrelendi,
- var olan oyun ustune yazma riski kaldirildi.
